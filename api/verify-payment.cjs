...
// Initialize Firebase Admin (bypasses allow write: if false securely)
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // 1. Fetch official real payment status from Razorpay API
        ...
        if ((payment.status === 'captured' || payment.status === 'authorized') && Number(payment.amount) === expectedAmountPaise) {
            ...
            return res.status(200).json({ status: 'success', message: 'Real payment verified and saved!' });
        } else {
